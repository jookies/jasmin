...
func main() {
	if err := run(); err != nil {
		log.Printf("synevyr-gateway: %v", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	jasminConfigPath := flag.String("legacy-cfg", "", "optional jasmin.cfg supplying infrastructure settings (broker, redis, listeners, thrower/dlr/smpps policy); connectors and routes still come from --config")
	...
	if runtimeConfig.HA != nil && runtimeConfig.HA.StandbyListenAddress != "" {
		...
		go func() {
			if serveErr := standbyServer.Serve(standbyListener); serveErr != nil && serveErr != http.ErrServerClosed {
				log.Printf("synevyr-gateway standby health: %v", serveErr)
			}
		}()
		log.Printf("synevyr-gateway waiting for leadership; standby health listening on %s",
	...
	if https != nil {
		log.Printf("synevyr-gateway listening on %s (TLS)", runtimeConfig.Outbound.ListenAddress)
	} else {
		log.Printf("synevyr-gateway listening on %s", runtimeConfig.Outbound.ListenAddress)
	}
	...
	if runtime.WebListenAddress != "" {
		...
		log.Printf("synevyr-gateway admin UI listening on %s", runtime.WebListenAddress)
	}
	...
	if runtime.AdminAPIListenAddress != "" {
		...
		log.Printf("synevyr-gateway admin API listening on %s", runtime.AdminAPIListenAddress)
	}
	...
	if runtime.RESTListenAddress != "" {
		...
		log.Printf("synevyr-gateway REST compatibility daemon listening on %s", runtime.RESTListenAddress)
	}
