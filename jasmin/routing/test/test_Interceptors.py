...
class InterceptorStrTestCase(InterceptorTestCase):
    def test_StaticMTInterceptor(self):
        ...
        self.assertEqual(str(s), 'StaticMTInterceptor/<IS (pyCode=print \'som ..)>')
    
    def test_StaticMOInterceptor(self):
        s = StaticMOInterceptor(self.simple_filter_mo, self.script1)
        self.assertEqual(str(s), 'StaticMOInterceptor/<IS (pyCode=print \'som ..)>')
    
    def test_DefaultInterceptore(self):
        s = DefaultInterceptor(self.script1)
        self.assertEqual(str(s), 'DefaultInterceptor/<IS (pyCode=print \'som ..)>')
