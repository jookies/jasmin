...
class UserAndCredentialsTestCase(TestCase):
    ...
    def test_without_user_defined_credentials(self):
        ...
        self.assertEqual(u.mt_credential.getValueFilter('source_address'), re.compile(b'.*'))
...
class MtMessagingCredentialTestCase(TestCase):
    def test_normal_noargs(self):
        ...
        self.assertEqual(mc.getValueFilter('destination_address'), re.compile(b'.*'))
        self.assertEqual(mc.getValueFilter('source_address'), re.compile(b'.*'))
        self.assertEqual(mc.getValueFilter('priority'), re.compile(b'^[0-3]$'))
        self.assertEqual(mc.getValueFilter('content'), re.compile(b'.*'))
