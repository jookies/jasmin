...
- Fill os ENV:
    + DB_TYPE_MYSQL         ## 1 for MySql, 0 for PostgreSQL
    + DB_HOST               ## IP or Docker container name
    + DB_DATABASE           ## should Exist
    + DB_TABLE              ## the script will create it if it doesn't Exist
    + DB_USER               ## for the Database connection.
    + DB_PASS               ## for the Database connection
    + AMQP_BROKER_HOST      ## RabbitMQ host used by Jasmin SMS Gateway. IP or Docker container name
    + AMQP_BROKER_PORT      ## RabbitMQ port used by Jasmin SMS Gateway. IP or Docker container name
...
def gotConnection(conn, username, password):
    ...
    db_type_mysql = int(os.environ['DB_TYPE_MYSQL']) == 1
...
if __name__ == "__main__":
    ...
    if int(os.environ['DB_TYPE_MYSQL']) == 1:
        print('*** Staring sms_logger, DB drive: MySql', flush=True)
    else:
        print('*** Staring sms_logger, DB drive: PostgreSQL', flush=True)
    print('**********************************************', flush=True)
    
    amqp_broker_host = os.environ['AMQP_BROKER_HOST']
    amqp_broker_port = os.environ['AMQP_BROKER_PORT']
    
    host = amqp_broker_host
    port = int(amqp_broker_port)
    vhost = '/'
    username = 'guest'
    password = 'guest'
