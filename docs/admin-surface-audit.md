...
| CDR, delivery outcome, billing outcome, or export | **Absent.** There is no CDR command (`internal/app/jcli/dispatch.go:14-18`). | **Absent.** There is no CDR route (`internal/app/admin/handler.go:38-51`). | **Absent.** The BFF has no CDR route (`internal/app/adminweb/server.go:130-209`), although the core service supports authorized record reads and CSV/JSONL export (`internal/core/cdr/operations.go:161-211`). |
| Guided partner onboarding | **Absent.** There is no onboarding command (`internal/app/jcli/dispatch.go:14-18`). | **Absent.** There is no onboarding route (`internal/app/admin/handler.go:38-51`). | **Partial, prototype only.** It is browser-tab state with no API calls and explicitly creates no credentials, routes, users, or connectors (`web/src/pages/partner-onboarding.tsx:210-233`, `web/src/pages/partner-onboarding.tsx:239-265`). |
| Invoice or customer statement generation | **Absent.** There is no billing-document command (`internal/app/jcli/dispatch.go:14-18`). | **Absent.** There is no billing-document route (`internal/app/admin/handler.go:38-51`). | **Absent.** No such BFF route or page is registered (`internal/app/adminweb/server.go:130-209`, `web/src/App.tsx:241-256`). |
...
`internal/app/outbound/runtime.go:1036-1047`). jCli accepts both class names but
...
5. **MT pool labels promise selection modes the data model does not store.**
   ...
   ordered first-available (`internal/app/outbound/runtime.go:1036-1047`).
