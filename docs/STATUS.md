...
1. **Commercial correctness / CDR.** Already working: route rate × parts charged at submit (`internal/core/submit_service.go:319`, `AuthorizeAndApplyCalculatedSubmit`), early-decrement percent, a late-billing consumer with a durable ledger, balance and `submit_sm_count` quota enforcement, group state, golden/enforcement tests, and — as of 2026-07-28 — durable balances that survive a restart (`internal/core/billing/quota_persister.go`, ADR-004). An earlier version of this line claimed "persistence" was working; it was not. `NewQuotaPersistenceService` and `NewPersistWorker` had zero production callers, so every prepaid balance reset to its provisioned value on each restart — a customer who had spent 900 of 1000 got the full 1000 back on every deploy. **Genuinely missing:** a release-grade prepaid/postpaid/user-group differential E2E through HTTP and SMPPs, and multi-node safety (the live balance is still per-process, so two nodes serving one account double-spend — see ADR-004). **CDRs are deliberately in the backlog:** the Python oracle has no CDR feature at all (`grep -rn cdr jasmin/` is empty), so they are greenfield design rather than parity, and plan 015 gate G2 explicitly permits excluding them from first commercial scope.
