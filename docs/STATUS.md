...
1. **Billing engine (#1) — NEXT.** Today the submit path carries the `SubmitSmBill` pickle and reads the late-bill amount from the AMQP header, and credential/quota checks exist at the submit boundary (`ErrQuotaExceeded`). The full rating/balance/prepaid-postpaid/CDR engine is unbuilt. Was de-prioritized until after #3.3, which is now done.
2. **jCli management console.** The interactive telnet management CLI is frozen as a spec (`spec/compatibility/JCLI_MATRIX.md`) but has no Go implementation — operators still use the legacy Python `jcli` for scripted management. Needed for full operational parity.
