...
2. **jCli management console — in progress, now fixture-backed.** `internal/app/jcli` implements the session/auth/help/completion layer and the `group` and `user` managers, all replaying **byte-for-byte** against transcripts captured from the frozen console (`spec/compatibility/fixtures/jcli/`, 9 of 18 fixtures green). Remaining: `smppccm`/`mtrouter`/`morouter` mutating verbs, `filter`, `httpccm`, the two interceptor managers, `stats`, `persist`/`load`, and `--smpp-unbind`/`--smpp-ban`. Two decisions are still open — a named-filter registry (routes reference filters by name, which reverses deviation D-001/ADR-003) and the `persist`/`load` semantics (plan 013 Step 7).
