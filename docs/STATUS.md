...
- **Live topology map.** A `/topology` console page draws the running gateway as one auto-generated graph — ingress, accounts, MT/MO routes, broker queues, SMPP and termination connectors, throwers, carriers and infrastructure — from a server-owned document (`GET /api/topology`) that joins configuration to live state. It exists for the faults no table view can show, because they span two pages: a route into a connector that is not bound, a `dlr-url` accepted while the thrower is off, a `submit.sm.<cid>` queue growing behind a connector that stopped consuming. Read-only, with deep links into the page that owns each entity. Layout is memoized on a structure hash that excludes every metric, so five-second polls repaint numbers in place instead of reshuffling the canvas. `stats.PrometheusRegistry` gained `Snapshot()` — an in-process read of the same series `/metrics/prometheus` publishes, with a test asserting the two can never disagree. It also draws the downstream delivery path the console never had: the signed HTTP push per termination connector, the message spool, and each customer's pull token graded by silence (warning at 15 min, red at an hour, with "issued but never used" and refused reads as their own signals) — the only delivery path with no connection to observe. `Scope.Connectors` supplies the edges, so the map answers which customer receives which connector's traffic. Verified on a running stack: a connector pointed at a black-holed address produced exactly one broken path and one unbound entry; a freshly issued token read "never pulled" until one real authenticated pull moved it to healthy. `scripts/dev-seed-topology.py` provisions a heavy, deliberately broken fixture for testing it. — plan 025, ADR-009
- **Billing console.** The web UI has a Billing section over the durable commercial records that no surface previously reached: per-customer granted-versus-remaining balance and quota (with the group's shared ceiling), CDR search by customer and window with the immutable event timeline per part, cursor-paged CSV/JSONL export, SQL-side rated usage statements separating money charged from late money still quoted, and a read-only settings card with reconcile and a confirmation-gated retention prune. Every read is audited as the logged-in operator. Two misleading numbers were corrected on the way: the user list showed the provisioned grant as if it were the live balance, and the rate quote ignored its destination, returning the highest-order route's rate for everyone. Verified on a running stack — a charged send moves remaining below granted, and the statement total equals the balance delta. — plan 019
