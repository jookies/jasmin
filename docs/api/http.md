...
| `sdt` | string | no | Fifteen decimal digits followed by `+`, `-`, or `R` (`internal/transport/httpcompat/handler.go:34`). **Known defect:** a syntactically valid value is not parsed; it is replaced with the gateway's current time (`internal/transport/httpcompat/handler.go:455`, `internal/transport/httpcompat/handler.go:466`). Do not use this parameter for scheduling. |
