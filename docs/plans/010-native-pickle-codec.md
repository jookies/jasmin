...
- **Date:** 2026-07-27
- **Status:** done — all 10 bridge actions ported to `internal/transport/gopickle` + `picklecompat.NativeCodec` (machine #93; connectors+response #94; submit encode #95; submit decode #96; repickle #97; deliver encode #98; routed decode #99; SubmitSmBill #100), each proven `pickle.loads`-equal to the live bridge. The gateway selects the engine via `pickle_codec: native|bridge` (default `bridge`); the native path passes the full gateway integration test E2E (HTTP submit → durable AMQP → native decode → SMPP multipart submit → submit_sm_resp → DLR, no subprocess). Follow-ups: flip the default to `native` after soak and drop the pickle-bridge layer from the image (the interceptor runner keeps Python until it too is ported); schedule/validity times, custom TLVs and GSM data_codings are deferred (they poison natively, matching the bridge allowlist edges).
