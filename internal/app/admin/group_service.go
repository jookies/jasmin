...
func NewGroupService(store *Store, provisioner GroupProvisioner, now func() string) (*GroupService, error) {
	...
	return &GroupService{store: store, provisioner: provisioner, now: now}, nil
}
...
// DeleteGroup removes an admin group from the directory and the store.
func (s *GroupService) DeleteGroup(ctx context.Context, gid string) error {
	...
	return s.store.DeleteGroup(ctx, gid)
}
