...
EXPECTED_MACRO_COUNTS = {"MS-1": 38, "MS-2": 12, "MS-3": 7, "MS-4": 9, "MS-5": 31, "MS-6": 10, "MS-8": 10, "MS-9": 34}
...
def validate_evidence(root: Path, evidence_dir: Path, ownership: Sequence[Mapping[str, str]], *, scope: str | None = None, mode: str | None = None, trusted_public_key: Path | None = None) -> None:
    ...
    if trusted_public_key is None:
        raise RegistryError("candidate evidence trusted public key must be supplied from outside the candidate repository")
    ...
    try:
        ...
        raise RegistryError("candidate evidence trusted public key must be outside the candidate repository")
    try:
...
def validate_repository(root: Path, evidence_dir: Path | None = None, *, scope: str | None = None, mode: str | None = None, trusted_public_key: Path | None = None) -> Registry:
    ...
    if evidence_dir is not None:
        validate_evidence(root, evidence_dir, ownership, scope=scope, mode=mode, trusted_public_key=trusted_public_key)
    return registry
